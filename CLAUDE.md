...
| Path | Purpose |
|---|---|
| `email-template.html` | Email 1 — Priority Access |
| `email-2-disconnected.html` | Email 2 — Where firm visibility breaks down |
| `email-3-how-spm-works.html` | Email 3 — From lead to client value, in one workflow |
| `email-4-supported-rollout.html` | Email 4 — Supported rollout for selected firms |
| `email-5-close-the-loop.html` | Email 5 — Close the loop |
| `email-styles.css` | **Single CSS source of truth for all 5 emails.** Edit here, then run `python3 build.py` |
| `build.py` | Inlines `email-styles.css` into each email's `<style>` block |
| `playground.html` | Multi-viewport preview UI (Desktop · Mobile, scroll-synced) |
| `SHIPPING.md` | **Pre-PR checklist.** Read before any email PR. Asset uploads, CSS rebuild, banned-phrase grep, real-client check |
| `COPY_GUIDE.md` | Voice & copy guide with running feedback log. Read before writing any new email |
| `BRAND.md` | Canonical color palette and voice rules — **always reference this for any visual change** |
| `.github/workflows/pages.yml` | Deploys `playground.html` + templates to GitHub Pages |

## Editing styles

The `<style>` block inside every `email*.html` is generated by `build.py` from `email-styles.css`. **Do not edit it by hand** — the next build will overwrite your changes.

```bash
# 1. Edit email-styles.css
# 2. Run the build to fan out to all 5 emails
python3 build.py
# 3. Commit both the CSS file AND the regenerated HTML files
```

## Before any email PR

Read `SHIPPING.md`. The non-negotiables in one block:

```bash
grep -n "TODO_" email*.html     # must return zero (upload assets, swap URLs first)
grep -n "—" email*.html          # must return zero (em dashes are the strongest AI tell)
python3 build.py                 # if you touched email-styles.css
```
...
- **Table-based layout, inline styles.** Gmail/Outlook/Apple Mail require it. Resist the urge to convert to semantic HTML or external stylesheets. The shared `<style>` block in each email comes from `email-styles.css` via `build.py`; per-element styles stay inline.
